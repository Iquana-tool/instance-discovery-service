...
async def list_models():
    """ Lists all available integrated_models in the registry. """
    available_models = MODEL_REGISTRY.get_model(only_return_available=False)
    return {
        "success": True,
        "message": f"Retrieved {len(available_models)} available integrated_models.",
...
async def load_model(model_registry_key: str, user_id: str):
    """ Loads a model into the cache if not already loaded. This is a convenience endpoint; integrated_models are loaded
